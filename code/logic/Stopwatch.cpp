...
namespace logic {
Stopwatch::Stopwatch()
    : lastTime(clock::now()), lastTimeEaten(clock::now()), deltaTime(0.0f), LastTimeSindsSec(clock::now()) {
}
...
bool Stopwatch::decScore() {
    auto currentTime = clock::now();
    const std::chrono::duration<float> elapsed = currentTime - LastTimeSindsSec;
    if (elapsed.count() > 1.0f) {
        LastTimeSindsSec = currentTime;
        return true;
    }
    return false;
}
