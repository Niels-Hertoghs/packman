...
std::pair<std::vector<sf::Text>, std::vector<sf::RectangleShape> > menuState::run(
    ...
    highNumbText.setOrigin(highNumbBounds.width / 2.f, highNumbBounds.height / 2.f);
    // het midden van de tekst is de coo waar het staat
...
std::pair<std::vector<sf::Text>, std::vector<sf::RectangleShape> > pausedState::run(
    ...
    // de linker boven hoek van de buttom altijd in de linkerbovenhoek van het scherm zetten
    rechthoeken.push_back(backToMenuButton);
