...
namespace view {
    sf::Text makeText(const sf::Font &fontF, const std::string &text, const double charSize, const sf::Color color,
                      const double x, const double y, const camera &cam) {
        sf::Text Text;
        Text.setFont(fontF);
        Text.setString(text);
    ...
    sf::RectangleShape makeButton(const float height, const float width, const sf::Color kleur, const camera &cam,
